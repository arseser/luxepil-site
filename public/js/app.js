document.addEventListener('DOMContentLoaded', function () {
  ...
  async function loadSalons() {
    ...
    try {
      ...
      if (salonsData.length === 0) {
        salonsTabs.innerHTML = '<p style="text-align:center;color:red;">Нет данных о салонах</p>';
        return;
      }
      ...
      salonsTabs.innerHTML = '<p style="text-align:center;color:red;">Ошибка загрузки салонов</p>';
    }
  }
  ...
  function renderSalon(salonId) {
    ...
    const lat = coords[0];
    const lng = coords[1];
    mapImage.src = `https://api-maps.yandex.ru/services/static?ll=${lng},${lat}&z=16&l=map&size=600,300&pt=${lng},${lat},pm2rdl`;
    mapImage.alt = `Карта к ${salon.name}`;
  ...
  async function loadServices() {
    ...
    try {
      ...
      if (!data.categories || data.categories.length === 0) {
        serviceTabs.innerHTML = '<p style="text-align:center;color:red;">Нет категорий услуг</p>';
        return;
      }
      ...
      console.error('❌ Ошибка:', err);
      serviceTabs.innerHTML = '<p style="text-align:center;color:red;">Ошибка загрузки услуг</p>';
    }
  }
