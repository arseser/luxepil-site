...
// ----- Логирование заявок (маскировка телефона) -----
const BOOKINGS_FILE = path.join(__dirname, 'data', 'bookings.json');
function logBooking(data) {
  try {
    let logs = [];
    if (fs.existsSync(BOOKINGS_FILE)) {
      logs = JSON.parse(fs.readFileSync(BOOKINGS_FILE, 'utf8'));
    }
    const maskedPhone = data.phone ? data.phone.slice(0, 4) + '******' : 'не указан';
    const logEntry = {
      ...data,
      phone: maskedPhone,
    ...
    logs.push(logEntry);
    fs.writeFileSync(BOOKINGS_FILE, JSON.stringify(logs, null, 2));
  } catch (e) {
    console.error('Не удалось записать лог заявки:', e.message);
  }
}
...
app.post('/api/booking', async (req, res) => {
  ...
  if (!botToken || !chatId) {
    logBooking({ name, phone, service, master, date, time, comment, status: 'failed_no_telegram' });
...
✉️ Комментарий: ${comment || '—'}
  ...
  try {
    ...
    logBooking({ name, phone, service, master, date, time, comment, status: 'sent' });
    res.json({ success: true, message: 'Заявка отправлена!' });
  } catch (error) {
    console.error('Telegram error:', error);
    logBooking({ name, phone, service, master, date, time, comment, status: 'failed_telegram', error: error.message });
